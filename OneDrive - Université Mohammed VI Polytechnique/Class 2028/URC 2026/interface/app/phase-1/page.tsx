...
import WheelHistoryComponent from '@/components/WheelHistory';

export default function Phase1() {
  const { teams, wheelHistory } = useTeams();
  ...
  return (
    <main className="h-screen text-cyan-300 font-mono overflow-hidden flex flex-col relative z-10">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex gap-2 ml-4">
            ...
            {wheelHistory.length > 0 && (
              <button
                onClick={() => setShowWheelHistory(!showWheelHistory)}
                className="border-2 border-yellow-400 px-3 py-1 text-yellow-300 font-bold text-xs cursor-pointer hover:bg-yellow-400 hover:text-blue-950 transition-colors bg-transparent whitespace-nowrap"
              >
      ...
      <div className="flex-1 relative z-20 overflow-hidden">
        <RaceManager teams={teams} showLeaderboard={showLeaderboard} setShowLeaderboard={setShowLeaderboard} phase="phase-1" />
      </div>

      {/* Wheel History Modal */}
      {showWheelHistory && (
        <div onClick={() => setShowWheelHistory(false)}>
